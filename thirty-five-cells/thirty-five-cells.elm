...
enemyMovement ( _, enemyRow, enemyCol ) ( _, playerRow, playerCol ) =
  ...
  let
    dRow =
      trimDifference enemyRow playerRow

    dCol =
      trimDifference enemyCol playerCol

    movement =
      case (playerRow + playerCol) % 5 of
...
advanceTick game =
  if collides game.enemy game.player then
    ...
    { game
      | enemy = move game.enemy game
      , player = move game.player game
    }
  ...
  case game.state of
    ...
    Start ->
      case msg of
        ...
        _ ->
          ( ( oldTime, game ), Cmd.none )

    GameOver ->
      case msg of
        KeyMsg 32 ->
          let
            ( ( initTime, initModel ), _ ) =
              init
          in
...
renderEntity game ( entityType, row, col ) =
  let
    colWidth =
      100 / toFloat game.nCols

    rowHeight =
      100 / toFloat game.nRows

    x =
      colWidth * toFloat col

    y =
      rowHeight * toFloat row

    content =
      case entityType of
        Player ->
          renderDirection game.playerDirection
